...
const syntax = BNF.types.BNF_Tree.fromJSON(
	JSON.parse(fs.readFileSync(path.resolve(__dirname, "../qupa.json"), 'utf8'))
);
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
	...
	if (result.hasError || result.isPartial) {	
		...
		let diagnostic: Diagnostic = {
			...
			range: {
				start: textDocument.positionAt(result.tree.ref.end.index),
				end: textDocument.positionAt(ref.index)
			},
			...
			diagnostic.relatedInformation = [
				{
					...
					message: `${result.tree.reached.getCausation(true)}`
				}
			];
		}
